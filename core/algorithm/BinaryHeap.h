...
namespace Etoile
{
	...
	public:
		BasicBinaryHeap(){}
		std::vector<Comparable*>& getHeap(){return _heap;}
		Comparable* getBinaryHeapNodeByIndex(unsigned int index)
		{
			return _heap[index];
		}
		virtual void swim(int index) = 0;
		virtual void sink(int index) = 0;

		void insertHeapNode(Comparable* node) 
		{
			_heap.push_back(node);
			swim(_heap.size() - 1); //heapifyUp
		}
	...
	class MaxBinaryHeap : public BasicBinaryHeap
	{
	public:
		...
		**/
		void swim(int index)
		{
			int parentIdx = getParentNodeIndex(index);
			while( ( index > 0 ) && ( parentIdx >= 0 ) && ( _heap[parentIdx] < _heap[index] ) )
			{
				Comparable* tmp = _heap[parentIdx];
				_heap[parentIdx] = _heap[index];
				_heap[index] = tmp;
				index = parentIdx;
				parentIdx = getParentNodeIndex(index);
			}
		}
	...
	class MinBinaryHeap : public BasicBinaryHeap
	{
	public:
		...
		**/
		...
		{
			int parentIdx = getParentNodeIndex(index);
			while( ( index > 0 ) && ( parentIdx >= 0 ) && ( _heap[index] < _heap[parentIdx] ) )
			{
				Comparable* tmp = _heap[parentIdx];
				_heap[parentIdx] = _heap[index];
				_heap[index] = tmp;
