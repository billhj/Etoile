...
namespace Etoile
{
	...
	void Joint::update()
	{
		...
		{
			_worldPosition = getParent()->getWorldRotation() * Vec3f(_translateFromParent.x() , _translateFromParent.y(), _translateFromParent.z() );
	...
	/*Vec3f Joint::transformFromOriginalGlobalToGlobalPosition(Vec3f global)
	{
		...
		return  _worldPosition + _worldRotation * Vec3f( localPoint.x(), localPoint.y(),  localPoint.z());
	}
