...
while ~strcmp( C{ii} , 'MOTION' )
  ...
  elseif strcmp( token , 'OFFSET' )
    ...
    ii = ii+3;
    if skeleton(nn).parent ~= 0
        skeleton(nn).worldposition = skeleton(nn).offset + skeleton(skeleton(nn).parent).worldposition;
    else
        skeleton(nn).worldposition = skeleton(nn).offset;
    end
